...
export class DocumentService {
  ...
  // // BehaviorSubject to hold and emit the document list
  // private documentsSubject = new BehaviorSubject<UserDocument[]>([]);
  // documents$ = this.documentsSubject.asObservable();

  // constructor(private http: HttpClient) { }

  // // Method to get documents by user ID
  // getDocuments(userid: number): Observable<UserDocument[]> {
  //   const headers = new HttpHeaders().set('Authorization', `Bearer ${localStorage.getItem('token')}`);
  //   return this.http.get<UserDocument[]>(`${this.getDocumentsUrl}/${userid}`, { headers });
  // }
  ...
  // // Load and update documents list
  // loadDocuments(userid: number): void {
  //   this.getDocuments(userid).subscribe(
  //     data => this.documentsSubject.next(data),
  //     error => console.error('Error loading documents', error)
  //   );
  // }

  // // Method to refresh documents list
  // refreshDocuments(userid: number): void {
